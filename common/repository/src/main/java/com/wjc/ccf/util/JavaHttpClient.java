...
/**
 * @author wangjunce 2018/10/29 11:07
 */
public class JavaHttpClient extends AbstractHttpClient implements HttpClient{
    ...
    private HttpURLConnection openConnection(URL url) throws IOException {
        if (isUseProxy()) {
            return (HttpURLConnection)url.openConnection(proxy);//(new Proxy(Proxy.Type.HTTP, new InetSocketAddress(getHostname(), getPort())));
        } else {
